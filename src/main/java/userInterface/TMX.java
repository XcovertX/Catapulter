...
public class TMX {
	...
	public TMX( String path ) {
		...
		try {
			...
			e.printStackTrace();
			assert false: "ERROR: (TMX) Failed to build '" + dbFactory.toString() + "'";

		} catch ( IOException e ) {

			e.printStackTrace();
			assert false: "ERROR: Could not parse file '" + path + "'";
		...
		for( int i = 0; i < getTilesets().getLength(); i++ ) {
			...
			getTSX_paths()[ i ] = source.getNodeValue().toString();
		}
	...
	public String[] getTSXPaths() {
		...
		try {
			...
			e.printStackTrace();
			assert false: "ERROR: (TMX) Failed to build '" + dbFactory.toString() + "' to retrieve TSXPaths.";
		}
		...
		for( int i = 0; i < imagePaths.length; i++ ) {
			...
			try {
				...
				e.printStackTrace();
				assert false: "ERROR: (TMX) Could not parse file '" + tsx_paths[ i ] + "'";

			} catch ( SAXException e ) {

				e.printStackTrace();
				assert false: "ERROR: (TMX) SAX Could not parse file '" + tsx_paths[ i ] + "'";
			}
...
//			imagePaths[ i ] = file.getParent() + "/";
	...
	public NodeList getTilesets() { return tileSets; }

	public void setTilesets( NodeList tilesets ) { this.tileSets = tilesets; }
	...
	public NodeList getLayerList() { return layerList; }

	public void setLayerList( NodeList layerList ) { this.layerList = layerList; }
	...
	public ArrayList< ObjectImage > getObjs() { return objs; }

	public void setObjs( ArrayList< ObjectImage > objs ) { this.objs = objs; }

	public String[] getTSX_paths() { return tsx_paths; }

	public void setTSX_paths( String[] tsx_paths ) { this.tsx_paths = tsx_paths; }
}
